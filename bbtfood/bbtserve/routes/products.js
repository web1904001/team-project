...
router.get("/",(req,res)=>{
  ...
  if(kw){
    ...
    var sql=`select *,(select md from bbt_laptop_pic where laptop_id=lid limit 1) as md from bbt_laptop where ${where}`;
    query(sql,[])
    .then(result=>{
...
router.get("/shelp",(req,res)=>{
  ...
  var where=kws.join(" and ");
  var sql=`select lid,title from bbt_laptop where ${where} limit 10`;
